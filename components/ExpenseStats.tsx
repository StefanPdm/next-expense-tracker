...
const ExpenseStats = async () => {
  // Fetch both average and range data with error handling
  const [userRecordResult, rangeResult] = await Promise.all([
    getUserRecord().catch(() => null),
    getBestWorstExpense().catch(() => null),
  ]);

  // Early return if data fetch failed
  if (!userRecordResult || !rangeResult) {
    return (
      <div className='bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm p-4 sm:p-8 rounded-2xl shadow-xl border border-gray-100/50 dark:border-gray-700/50'>
        <div className='flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6'>
          <div className='w-8 h-8 sm:w-12 sm:h-12 bg-gradient-to-br from-emerald-500 via-green-500 to-teal-500 rounded-xl flex items-center justify-center shadow-lg'>
        ...
        <div className='bg-red-50/80 dark:bg-red-900/20 backdrop-blur-sm p-4 sm:p-6 rounded-xl border-l-4 border-l-red-500'>
          <div className='flex items-center gap-3 mb-2'>
            <div className='w-6 h-6 sm:w-8 sm:h-8 bg-red-100 dark:bg-red-800 rounded-full flex items-center justify-center'>
          ...
          <p className='text-red-700 dark:text-red-400 text-xs sm:text-sm ml-9 sm:ml-11'>
