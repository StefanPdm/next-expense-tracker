...
async function addExpenseRecord(formData: FormData): Promise<RecordResult> {
  ...
  try {
    ...
    // Nach dem erfolgreichen Hinzufügen eines neuen Expense-Records
    // const createdRecord = await db.record.create({ ... });
    // Invalidiere den Cache der Homepage, damit die neue Ausgabe sofort angezeigt wird
    // Warum ist das wichtig?
    // Next.js cached standardmäßig Seiten für bessere Performance. Ohne revalidatePath() würde
    //  der neue Expense-Record erst nach Ablauf der Cache-Zeit oder einem manuellen Reload sichtbar werden.
    //  Mit dieser Funktion stellst du sicher, dass die Änderungen sofort für alle Benutzer sichtbar sind.
    revalidatePath('/');
